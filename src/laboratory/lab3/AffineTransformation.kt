...
class AffineTransformation(private var image: Mat) {
    fun move(movingCoif: Double): Mat {
        ...
        return if (checkMaxSize(image.cols() + movingCoif, image.rows() + movingCoif)){
            println("This image is too big for moving!")
            image
        } else {
    ...
    fun turn(degree: Double, scale: Double): Mat {
        ...
        val a2 = image.cols() * image.cols()
        val b2 = image.rows() * image.rows()
        val dig = sqrt((a2 + b2).toDouble())

        return if (checkMaxSize(dig, dig)){
            println("This image is too big for turning/scaling!")
            image
        } else {
            Imgproc.warpAffine(
                image,
    ...
    private fun checkMaxSize(width: Double, height: Double): Boolean {
